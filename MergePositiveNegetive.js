function mergeFunction() {
	...
	var grid;
	var columnFilters = {};
	var data = document.getElementById("mergeDiv").value;
	console.log(data);
	...
	var resendMessage = function(row, cell, value, columnDef, dataContext) {
		let phoneNo = dataContext.testMobileNo;
		console.log(phoneNo);
		let patientName = dataContext.testPatientName;
		let patientReport=dataContext.testDataResult;
		return `<button id="sendmessage" onclick="yourFunction(`+ phoneNo + `,'` + patientName +`','` + patientReport  + `')">Send Message</button>`
	}
...
},
{
	...
	$(function() {
		...
		grid = new Slick.Grid("#mergeGrid", dataView, columns, options);
...
async function yourFunction(mobNo,name,report) {


	console.log(mobNo);
	console.log(name);
	console.log(report);
	let response = await fetch("http://localhost:8080/sendSms?mobileNumber=" + mobNo + "&name=" + name+"&report="+report,{ method: "POST" });
